...
import { LayoutDashboard, Calendar as CalendarIcon, Activity, Settings, Users } from 'lucide-react';
...
export const MobileNav = ({ currentView, setCurrentView }: MobileNavProps) => {
  const menuItems = [
    ...
    { id: AppView.COMMUNITY, icon: Users, label: 'المجتمع' }, // Added
    { id: AppView.SETTINGS, icon: Settings, label: 'إعدادات' },
  ];

  return (
    <div className="md:hidden fixed bottom-4 left-4 right-4 h-20 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.6)] z-50 flex items-center justify-between px-2 animate-in slide-in-from-bottom-20 duration-700">
      {menuItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span className={`absolute -bottom-6 text-[10px] font-bold tracking-wide transition-all duration-300 ${
